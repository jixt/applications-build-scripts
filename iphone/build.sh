...
for SDK in $SDKS; do
    for CONFIG in $CONFIGURATIONS; do
		...
		curl -3 $KOOMODA_API_URL -F file=@"${OUTPUT}/${IPA_NAME}" -F icon=@"${OUTPUT}/Icon-57.png" -F manifest=@"${OUTPUT}/${OTA_NAME}" -F user_token="'${K_ACCOUNT_TOKEN}'" -F app_token="'${K_ACCOUNT_TOKEN}'" -F app_version="'${BUILD_NUMBER}'"
    done
done
